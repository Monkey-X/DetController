...
public class ReportDetailActivity2 extends BaseActivity {
    ...
    private void initView() {
        ...
        //数据库里取数并展示
        String proId = detController.getContractId();
        XLog.e("proId: " + proId);
...
//            DBManager.getInstance().getReportEntityDao().insertOrReplace(reportEntity);
    ...
    private void initTableView(List<DetonatorEntity> detonatorList) {
        ...
        for (i = 0; i < size; i++) {
            ...
            detData[i][1] = detonatorList.get(i).getCode();
...
//            session.close();
    ...
    private void UPZBThread(List<Detonator> detonators) {
...
//        step = 0;
    ...
    private List<ReportDto> getReportDto(DetController detController) {
        XLog.e("detController: " + detController.toString());
        List<ReportDto> reportDtos = new ArrayList<>();
        List<Detonator> detList = detController.getDetList();
//        List<Detonator> detList = JsonUtils.monitDetonator();//模拟数据
