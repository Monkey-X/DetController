...
//            Toast.makeText(this, "没有数据", Toast.LENGTH_SHORT).show();
    ...
    @OnClick(R.id.author_query)
    public void getProjectFileContent() {
...
//                showToast("你输入的是: " + edit.getText().toString());
    ...
    private void getProjectFile(String fileSn) {
        ...
        params.put("dwdm", Globals.user.getCompanyCode());    // 输入单位代码
        params.put("xlh", fileSn);
        String newUrl;
        // 测试服务器
        if (Globals.isTest) {
...
//        XLog.v( "newUrl:" + newUrl);
        ...
        AsyncHttpCilentUtil.getOkHttpClient(newUrl, new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                showLongToast(fileSn + "onFailure！" + e.getLocalizedMessage());
...
//                showLongToast(fileSn + "下载成功！");
                // 保存原始的数据
                String fdName = "pf_" + DateUtil.getDateDoc(new Date()) + "-" + fileSn + ".json";
...
//                XLog.i( "projectFileDto:" + projectFileDto);
                Result detInfoResult = projectFileDto.parseContentAndSave(fileSn);
...
//                    projectFile.setMmwj("");
                    fdName =  Globals.user.getCompanyCode()+ "-" + fileSn+ "-" + DateUtil.getDateDoc(new Date()) + ".json";
