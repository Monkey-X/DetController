/**
  * Copyright 2019 Sommer
  */
package com.etek.controller.dto;


import java.util.Date;

public class Zbqy {

    private String zbqybj;  //准爆区域半径
    private String zbqymc;  //准爆区域名称
    private String zbqywd;  //准爆区域中心位置维度
    private Date zbqssj;  //准爆起始时间
    private String zbqyjd;  //准爆区域中心位置经度
    private Date zbjzsj;  //准爆截止时间
    public void setZbqybj(String zbqybj) {
         this.zbqybj = zbqybj;
     }
     public String getZbqybj() {
         return zbqybj;
     }

    public void setZbqymc(String zbqymc) {
         this.zbqymc = zbqymc;
     }
     public String getZbqymc() {
         return zbqymc;
     }

    public void setZbqywd(String zbqywd) {
         this.zbqywd = zbqywd;
     }
     public String getZbqywd() {
         return zbqywd;
     }

    public void setZbqssj(Date zbqssj) {
         this.zbqssj = zbqssj;
     }
     public Date getZbqssj() {
         return zbqssj;
     }

    public void setZbqyjd(String zbqyjd) {
         this.zbqyjd = zbqyjd;
     }
     public String getZbqyjd() {
         return zbqyjd;
     }

    public void setZbjzsj(Date zbjzsj) {
         this.zbjzsj = zbjzsj;
     }
     public Date getZbjzsj() {
         return zbjzsj;
     }

}